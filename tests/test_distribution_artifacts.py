...
class DistributionArtifactsTests(unittest.TestCase):
    ...
    def test_readme_documents_standalone_install_and_verification(self):
        ...
        self.assertIn("aos release-upgrade-smoke --repo-root . --from-ref v0.1.11-public-alpha --to-ref HEAD --json", content)
        self.assertIn("aos public-release-gate --repo-root . --json", content)
        self.assertIn("aos release-install-smoke --source https://github.com/Dai202703/agentic-os-public-alpha.git --ref v0.1.13-public-alpha --expected-tag v0.1.13-public-alpha --json", content)
    ...
    def test_distribution_doc_includes_handoff_verification_gate(self):
        ...
        self.assertIn("aos release-check --repo-root . --fresh-user-smoke --json", content)
        self.assertIn("aos release-check --repo-root . --upgrade-smoke --from-ref v0.1.11-public-alpha --to-ref HEAD --json", content)
        self.assertIn("aos public-release-gate --repo-root . --json", content)
        self.assertIn("aos release-install-smoke --source https://github.com/Dai202703/agentic-os-public-alpha.git --ref v0.1.13-public-alpha --expected-tag v0.1.13-public-alpha --json", content)
    ...
    def test_public_release_doc_documents_version_traceability(self):
        ...
        self.assertIn("aos version", content)
        self.assertIn("v0.1.13-public-alpha", content)
        self.assertIn("v0.1.12-public-alpha", content)
