...
class InstallationManagerTests(unittest.TestCase):
    ...
    def test_install_script_installs_launcher_to_env_install_dir(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(active.is_symlink())
            self.assertEqual(self.launcher.resolve(), active.resolve())
            self.assertIn("AOS version 0.1.15", install.stdout)
            self.assertIn("Release tag: v0.1.15-public-alpha", install.stdout)
            ...
            self.assertIn("4/5 temporary doctor skipped by AOS_INSTALL_SKIP_CHECKS=1", install.stdout)
            self.assertIn("5/5 version", install.stdout)
            self.assertIn("aos install complete", install.stdout)
            self.assertIn("Next: run 'aos init'", install.stdout)
